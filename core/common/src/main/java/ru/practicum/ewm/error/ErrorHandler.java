...
public class ErrorHandler {
    ...
    public ResponseEntity<ApiError> handleFeignException(final FeignException e) {
        ...
        if (status == null) {
            log.error("Feign error with unknown status: {}", e.getMessage());
            ApiError apiError = build(HttpStatus.INTERNAL_SERVER_ERROR, "Internal server error.", e.getMessage());
        ...
        if (!content.isBlank()) {
            try {
                ...
                log.error("Feign error propagated with status {}: {}", status, apiError.getMessage());
                return ResponseEntity.status(status).body(apiError);
            } catch (JsonProcessingException parseException) {
                log.error("Failed to parse Feign error body: {}", content, parseException);
            }
        }

        log.error("Feign error with status {}: {}", status, e.getMessage());
        ApiError apiError = build(status, status.getReasonPhrase(), e.getMessage());
        return ResponseEntity.status(status).body(apiError);
    }
